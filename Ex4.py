...
def load_data (path) -> pd.DataFrame: 
    ...
    my_sep = ',' if path=='haberman.data' else ' ' # different seperator for each dataset
    ...
    training_idx, test_idx = indices[:int(data.shape[0]*0.5)], indices[int(data.shape[0]*0.5):]
...
def knn_fit(x_train, y_train, k, p):
    def knn_predict_one(x_pred):
        ...
        # return magority of nighbors labels
...
if __name__ == "__main__":
    ...
    print('Dataset: Haberman Survival Dataset')
    for k in [1, 3, 5, 7, 9]:
        for p in [1, 2, -1]:
            empirical_err_sum = 0
            true_err_sum = 0
            for i in range(100):
                X_train, X_test, y_train, y_test = load_data("haberman.data")
            ...
            print(f'Difference between errors for k={k} lp= {lp}: {abs(empirical_err_sum/100-true_err_sum/100)}\n')
    
    print('Dataset: Squares Dataset')
    for k in [1, 3, 5, 7, 9]:
        for p in [1, 2, -1]:
            empirical_err_sum = 0
            true_err_sum = 0
            for i in range(100):
                X_train, X_test, y_train, y_test = load_data("squares.txt")
                knn_predict = knn_fit(X_train, y_train, k, p)
                y_test_pred = knn_predict(X_test)
                y_train_pred = knn_predict(X_train)
                empirical_err_sum += error(y_train_pred, y_train)
    ...
    # fake_X_train = np.array([[52, 68, 0],[52, 68, 0],[52, 68, 0]])
    # fake_y_train = np.array([2,2,1])
